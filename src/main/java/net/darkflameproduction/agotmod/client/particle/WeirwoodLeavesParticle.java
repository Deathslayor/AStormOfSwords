...
public class WeirwoodLeavesParticle extends TextureSheetParticle {
    ...
    private static final int SPRITE_VARIANTS = 1;
    ...
    protected WeirwoodLeavesParticle(ClientLevel level, double x, double y, double z,
                                     SpriteSet spriteSet) {
        super(level, x, y, z);
    ...
    public void tick() {
        ...
        double windX = Math.cos(Math.toRadians(this.particleRandom * 60.0F))
                * WIND_BIG * Math.pow(ageProgress, 1.25);
        ...
        if (this.onGround || this.lifetime < INITIAL_LIFETIME - 1
