...
public class MammothRenderer extends MobRenderer<MammothEntity, MammothRenderState, MammothModel> {
    ...
    @Override
    public void render(MammothRenderState state, @NotNull PoseStack poseStack, @NotNull MultiBufferSource buffer, int light) {
    ...
    @Override
    public @NotNull MammothRenderState createRenderState() {
        return new MammothRenderState();
    }

    @Override
    public void extractRenderState(@NotNull MammothEntity entity, @NotNull MammothRenderState state, float p_361157_) {
        super.extractRenderState(entity, state, p_361157_);
        state.idleAnimationState.copyFrom(entity.idleAnimationState);
        state.attackAnimationState.copyFrom(entity.attackAnimationState);
        state.variant = entity.getVariant();
        state.hasSaddle = entity.hasControllingPassenger();
    }

    @Override
    public @NotNull ResourceLocation getTextureLocation(MammothRenderState state) {
