...
public class SleepSystem {
    ...
    public SleepSystem(Peasant_Entity peasant) {
    ...
    public void tick() {
        ...
        // More aggressive bed search reset during sleep time
        if (peasant.shouldSleep() && !peasant.isSleeping() && !peasant.needsFoodCollection()) {
            ...
            // Much more aggressive cooldown management during sleep time
            if (!hasValidBed) {
        ...
        if (!level.isClientSide && bedPos != null) {
            ...
            if (!(bedState.getBlock() instanceof BedBlock)) {
                ...
                // If bed is destroyed during sleep time, immediately allow bed search
        ...
        if (!level.isClientSide && isSleeping() && !peasant.shouldSleep()) {
            stopSleeping();
        ...
        // Handle case where NPC is at sleep time but has no bed
        ...
        if (isSleeping()) {
            ...
            // Ensure NPC stays close to bed while sleeping
            if (bedPos != null) {
        ...
        // Additional safety check for stuck NPCs during sleep time
        if (!level.isClientSide && peasant.shouldSleep() && !peasant.isSleeping()) {
            ...
            if (peasant.getJobType().equals(JobSystem.JOB_GUARD)) {
                // For guards, use their shift-based sleep timing
                if (peasant.getGuardSystem().shouldSleep() && bedPos == null && bedSearchCooldown > 20) {
                    bedSearchCooldown = 0; // Force immediate bed search for guards
                }
            } else {
                // For regular NPCs, use standard sleep timing
                long sleepStartTime = 12542;
    ...
    public boolean shouldSleep() {
        ...
        // This method is overridden in Peasant_Entity for guards
    ...
    public void stopSleeping() {
        setSleeping(false);
        // Properly clear the sleeping pose when stopping sleep
    ...
    public static boolean isBedOccupied(Level level, BlockPos bedPos) {
        return level.getEntitiesOfClass(Peasant_Entity.class,
