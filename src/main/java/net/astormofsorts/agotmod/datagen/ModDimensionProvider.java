...
public class ModDimensionProvider {
    ...
    public final static URL BIOME_MAP_URL = AGoTMod.class.getResource("/assets/agotmod/map.png");
    public final static URL HEIGHT_MAP_URL = AGoTMod.class.getResource("/assets/agotmod/map_height.png");
    ...
    public static void bootstrapStem(BootstapContext<LevelStem> context) {
        ...
        List<MapBiomeData> biomeData = new ArrayList<>() {
            {
                add(formatBiomeData(biomeRegistry, Biomes.PLAINS, new Color(0, 255, 0), Blocks.DEEPSLATE, Blocks.STONE, Blocks.DIRT, Blocks.GRASS_BLOCK));
                add(formatBiomeData(biomeRegistry, Biomes.DESERT, new Color(255, 255, 0), Blocks.DEEPSLATE, Blocks.STONE, Blocks.SANDSTONE, Blocks.SAND));
                add(formatBiomeData(biomeRegistry, Biomes.OCEAN, new Color(0, 0, 255), Blocks.DEEPSLATE, Blocks.STONE, Blocks.WATER, Blocks.WATER));
            }
        };

        context.register(DIMENSION_STEM, new LevelStem(context.lookup(Registries.DIMENSION_TYPE).getOrThrow(DIMENSION_TYPE), new MapBasedBiomeChunkGenerator(new GeneratorSettings(biomeData, 0, 66, -32, 256, 54))));
    }
    ...
    private static MapBiomeData formatBiomeData(HolderGetter<Biome> biomeRegistry, ResourceKey<Biome> biome, Color color, Block deepSlateBlock, Block stoneBlock,
                                                Block dirtBlock, Block grassBlock) {
        return new MapBiomeData(biomeRegistry.getOrThrow(biome), color, ForgeRegistries.BLOCKS.getResourceKey(deepSlateBlock).orElseThrow(), ForgeRegistries.BLOCKS.getResourceKey(stoneBlock).orElseThrow(), ForgeRegistries.BLOCKS.getResourceKey(dirtBlock).orElseThrow(), ForgeRegistries.BLOCKS.getResourceKey(grassBlock).orElseThrow());
    }
}
